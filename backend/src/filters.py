    ...
    with open("imagesInfo.txt", 'w') as csvfile:
        ...
        for i in range(length):
            ...
            if (currentImgFlags[0] == 1):
                ...
                contrast = 2.3  
                image2 = cv2.addWeighted(img, contrast, np.zeros(img.shape, img.dtype), 0, 40)
            else: 
                contrast = 1.0
                image2 = cv2.addWeighted(img, contrast, np.zeros(img.shape, img.dtype), 0, 20)
            ...
            color_correction_matrix = np.array([[1.2, 0.1, 0.1],
                                        [0.0, 1.3, 0.2],
                                        [0.1, 0.2, 1.4]])
